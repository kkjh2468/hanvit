...
public class EventFrame extends Frame {
	...
	*/
	...
	public EventFrame() {
		...
		//final 변수 - 상수
		final int z = 10;
		//Frame의 기본 레이아웃이 BorderLayout 이라서 변경
		setLayout(new FlowLayout());
		//버튼 생성
		btn = new Button("버튼");
		
		//외부에 만든 ActionListener 인터페이스를 구현한 클래스의 객체를 생성해서 이벤트를 처리
		//생성자를 이용해서 데이터 넘기기
		//this 가 Frame 으로 부터 상속을 받았기 때문에 Frame 객체
		...
		//내부에 만든 ActionListener 인터페이스를 구현한 클래스의 객체를 생성해서 이벤트를 처리
		//btn.addActionListener(new EventHandler());
		
		//버튼을 클릭했을 때 처리를 위한 코드 구현
		//anonymous를 이용하는 방법
		//인스턴스 변수와 final을 사용이 가능하지만 이 코드가 구현된 메소드에서 생성한 지역변수는 사용을 못함
		/*
